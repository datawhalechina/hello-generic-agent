...
| `/restore` | 恢复上次对话 | 从最近一次保存的历史记录中恢复上下文，方便继续上次的工作 |
| `/resume` | 恢复历史对话（可选择） | 浏览最近几次对话的结尾摘要，让你选择要恢复哪一次，然后以该对话为基础继续聊天 |
...
2. **恢复上次对话**：重启 GA 后想继续之前的任务

   ```
   /restore      ← GA 自动找到最近的历史记录并恢复上下文
   ```
...
<details>
<summary>💡 / 命令在哪里可以用？</summary>

- **Qt 桌面窗口**：直接在输入框输入 / 命令即可
- **飞书 / 微信 / 钉钉等聊天平台**：在和 GA 的对话框中直接发送 / 命令
- 在 Qt 界面中，部分命令也有对应的按钮（如"清空对话"按钮等同于 `/new`，停止按钮等同于 `/stop`），两种方式效果完全一致

</details>

> ⚠️ **注意**：`/restore` 恢复的是最近一次的对话历史（保存在 `temp/model_responses_*.txt` 中）。恢复后 GA 只是获得了之前的上下文，我们需要输入新的问题来继续工作。
...
- **/ 命令**：`/help` `/status` `/stop` `/new` `/restore` `/llm` 六个快捷指令，高效控制 GA
- **对话技巧**：明确指代、分步确认、善用记忆和 `/restore`，让 GA 越用越懂你
